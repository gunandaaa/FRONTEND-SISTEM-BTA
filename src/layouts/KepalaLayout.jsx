...
function KepalaLayout() {
  // Tambahan state UI untuk responsivitas Mobile (seperti di MahasiswaLayout)
  const [isMobileOpen, setIsMobileOpen] = useState(false);

  // Kelas menu disesuaikan dengan palet BTA (Kuning menyala jika aktif)
  ...
  return (
    // Menggunakan h-screen agar layout tidak terpotong (Bug Fix)
    <div className="h-screen w-full bg-gray-50 flex font-sans text-bta-black overflow-hidden relative">
      ...
      >
        ...
        <div className="p-6 md:p-8 border-b border-white/10 flex justify-between items-center">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-bta-yellow text-xs font-bold uppercase tracking-wider mt-0.5">Kepala Pusat</p>
            </div>
          </div>
        ...
        <nav className="p-5 space-y-2 flex-1 overflow-y-auto">
          <NavLink to="/kepala/dashboard" onClick={closeMobileSidebar} className={menuClass}>
            Dashboard Eksekutif
          </NavLink>

          <NavLink to="/kepala/pengesahan-kelulusan" onClick={closeMobileSidebar} className={menuClass}>
            Pengesahan Kelulusan
          </NavLink>

          <NavLink to="/kepala/pelaporan" onClick={closeMobileSidebar} className={menuClass}>
            Pelaporan & Ekspor
          </NavLink>

          <NavLink to="/kepala/manajemen-pengguna" onClick={closeMobileSidebar} className={menuClass}>
            Manajemen Pengguna
          </NavLink>
        </nav>
        ...
        <header className="bg-white shadow-sm px-4 md:px-8 py-4 md:py-5 flex justify-between items-center z-10 border-b border-gray-100">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-gray-500 text-xs md:text-sm font-medium mt-0.5">Panel Eksekutif Kepala Pusat</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            <div className="text-right hidden sm:block">
              <p className="font-extrabold text-gray-800 tracking-tight">Pak Ulin</p>
              <p className="text-xs text-gray-500 font-bold uppercase tracking-wider mt-0.5">Kepala Pusat BTA</p>
            </div>
            {/* Avatar */}
