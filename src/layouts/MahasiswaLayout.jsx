...
function MahasiswaLayout() {
  // Menambahkan state khusus untuk mengontrol buka/tutup Sidebar di HP
  ...
  // Fungsi untuk menutup sidebar saat menu diklik (khusus di HP)
  const closeMobileSidebar = () => {
    setIsMobileOpen(false);
  };

  return (
    <div className="min-h-screen bg-gray-50 flex font-sans text-bta-black overflow-hidden relative">
      
      {/* ==========================================
          OVERLAY GELAP UNTUK MOBILE
          (Muncul saat sidebar terbuka, bisa diklik untuk menutup)
      ========================================== */}
      {isMobileOpen && (
        <div 
      ...
      >
        ...
        <div className="p-6 md:p-8 border-b border-white/10 flex justify-between items-center">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-bta-yellow text-xs font-bold uppercase tracking-wider mt-0.5">Mahasiswa</p>
            </div>
          </div>
        ...
        {/* Navigasi Menu */}
        <nav className="p-5 space-y-2 flex-1 mt-2 overflow-y-auto">
          {/* Tambahkan event onClick={closeMobileSidebar} agar menutup otomatis */}
          <NavLink to="/mahasiswa/dashboard" onClick={closeMobileSidebar} className={menuClass}>
            Dashboard
          </NavLink>

          <NavLink to="/mahasiswa/administrasi" onClick={closeMobileSidebar} className={menuClass}>
            Administrasi
          </NavLink>

          <NavLink to="/mahasiswa/kelasku" onClick={closeMobileSidebar} className={menuClass}>
            Kelasku
          </NavLink>

          <NavLink to="/mahasiswa/hasil-studi" onClick={closeMobileSidebar} className={menuClass}>
            Hasil Studi
          </NavLink>
        </nav>
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden w-full relative">
        ...
        <header className="bg-white shadow-sm px-4 md:px-8 py-4 md:py-5 flex justify-between items-center z-10 border-b border-gray-100">
          ...
          <div className="flex items-center gap-4">
            {/* Tombol Hamburger untuk memunculkan Sidebar di HP */}
            <button 
